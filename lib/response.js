...
class LambdaResponse {
  ...
  file(fname, filetype) {
    if (!filetype) {
      ...
      if (!filetype) {
        filetype = 'application/octet-stream' // Express default
      }
    }
    this.type(filetype)
    ...
    //}
    var base64str = fs.readFileSync(fname, 'base64')
    this.body = base64str
  ...
  base64(body, type) {
    // Express: Buffer object, a String, an object, or an Array
    // AWS S3: Buffer, Typed Array, Blob, String, ReadableStream
    //if ()
    this.isBase64Encoded = true
    this.body = body
    return this
